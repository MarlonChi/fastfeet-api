...
describe('ListRecipientItemsController', () => {
  ...
  beforeEach(async () => {
    ...
    controller = module.get<ListRecipientItemsController>(
  ...
  it('should return recipient and deliveries when valid email and documentId are provided', async () => {
    ...
    const result = await controller.handle(
      mockRecipient.email,
      mockRecipient.documentId,
    );

    expect(prismaService.recipient.findUnique).toHaveBeenCalledWith({
      where: { email: mockRecipient.email },
    });
    expect(prismaService.delivery.findMany).toHaveBeenCalledWith({
      where: { recipientId: mockRecipient.id },
    });
    expect(result).toEqual({
      recipient: {
        ...
        documentId: mockRecipient.documentId,
      },
  ...
  it('should throw BadRequestException if email or documentId is missing', async () => {
    await expect(controller.handle('', '123')).rejects.toThrow(
      BadRequestException,
    );
    await expect(controller.handle('test@email.com', '')).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('should throw ForbiddenException if recipient not found', async () => {
    ...
    await expect(
      controller.handle('nonexistent@email.com', '123'),
  ...
  it('should throw ForbiddenException if documentId does not match', async () => {
    ...
    await expect(
      controller.handle(mockRecipient.email, 'wrong-doc'),
    ).rejects.toThrow(ForbiddenException);
    await expect(
      controller.handle(mockRecipient.email, 'wrong-doc'),
